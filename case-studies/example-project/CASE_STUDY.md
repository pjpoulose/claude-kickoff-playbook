# Case Study: Example Project — April 2026
**Project type:** Consumer AI platform, pre-launch MVP
**Founder:** [anonymous] (employed full-time at [employer] during build)
...
[Project B] is an intelligent orchestration platform built as a side project while the founder is employed full-time. The product positions itself as "not AI — the layer that makes any AI yours." Key constraints: generic consumer positioning only (no [regulated industry] marketing during [employer] employment), no developer hired for Phase 0, ~25 hours/week available.
...
Maintaining a single file of locked decisions (with the exact table format) prevented Claude from re-debating settled questions. Once "self-hosted orchestration tool from Phase 0" was in the Decisions_Log, no session re-opened that question.
...
Building the morning brief using a lightweight workflow tool before setting up the full orchestration stack was recommended as a way to validate the core loop cheaply. The insight: the smallest thing that can tell you whether you'll use the product daily is worth building before spending thousands on infrastructure.
...
### A planned tool migration was avoided
The original plan included one workflow tool for Phase 0 and migration to a different tool at Phase 1. The discovery that Claude + MCP eliminated the second tool's learning curve made this migration unnecessary. Starting with the target tool from day one saved an estimated 20-40 hours of migration work.
